"""
Newton-Raphson Power Flow Solver (10,000 simulations)

Runs Example 3 from the IntechOpen reference 10,000 times with varying load at Bus 2
and varying generation at Bus 3. Saves results (P, Q, Voltage, Current, and Ybus) into separate CSV files.
...
for sim in range(iterations):
    ...
    # P3 = np.random.uniform(0.5, 3.5)
    P2 = -2.0
    Q2 = -0.5
    P3 = 1.5

    P_spec = np.array([0.0, P2, P3])
    Q_spec = np.array([0.0, Q2, 0.0])

    V = np.array([1.02, 1.0, 1.03])
    delta = np.zeros(3)
