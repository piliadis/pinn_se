...
for run in range(num_runs):
    ...
    if run == 0:
        ...
        Z_base = (np.array(V_base) ** 2) / S_BASE
    ...
    # === Validate Power Equation ===
    V_true = mag_V * np.exp(1j * ang_V_rad)  # [V]
    I_calc = Y_sparse @ V_true
    S_calc = V_true * np.conj(I_calc)
    S_true = np.array(real_S) + 1j * np.array(imag_S)
    if not np.allclose(S_calc, S_true, atol=1e-2, rtol=1e-3):
        print(f"[!] Power mismatch at run {run + 1}")
        print(f"Max error: {np.max(np.abs(S_calc - S_true))}")
        pdb.set_trace()
