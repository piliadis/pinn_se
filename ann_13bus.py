...
if __name__ == "__main__":
    ...
    torch.save(model.state_dict(), f"results/13bus/pinn_model{adjustment_step}.pth")
    ...
    logs.to_csv(f"results/13bus/loss_logs{adjustment_step}.csv", index=False)
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"results/13bus/error_curves{adjustment_step}.png")
    plt.show()
